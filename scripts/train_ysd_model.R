...
# Goal:
#   • Train models globally (Random Forest on BAP b1..b6, Random Forest on NBR1985)
#   • Apply locally to a single BAP tile (forest-only) in constant memory using block streaming
#   • Write out YSD (float + int), YOD, and CoE layers (median, agreement, IQR, SD) block-wise
# Design:
#   • No terra::predict/app parallel workers (avoids S3 + RAM blow-ups)
#   • Streaming I/O via readStart/readValues/writeValues/writeStop
#   • No EVI anywhere
# =======================================================================================================
...
# --------------------------- RESOURCES & TERRA OPTS --------------------------
RF_THREADS_TRAIN <- 30     # heavy during training
RF_THREADS_PRED  <- 4      # threads for ranger::predict on a *block* (keep modest)
Sys.setenv(OMP_NUM_THREADS="2", MKL_NUM_THREADS="2", OPENBLAS_NUM_THREADS="2", GDAL_NUM_THREADS="2")
terraOptions(progress = 1, memfrac = 0.6)   # push more to disk; adjust tempdir if you have a fast SSD
...
make_blocks <- function(r, chunk = 1024L) {
  ...
  nrows <- pmin(chunk, nr - rows + 1L)
  list(n = length(rows), row = rows, nrows = nrows)
}
to_int_vec <- function(v, lo=AGE_MIN, hi=AGE_MAX) {
  v <- round(v); v[v < lo] <- lo; v[v > hi] <- hi; v
}
agree_pairs_vec <- function(a, b, tol=1) {
  ok <- is.finite(a) & is.finite(b)
  res <- rep(NA_real_, length(a))
  res[ok] <- as.numeric(abs(a[ok] - b[ok]) <= tol)
  res
}
iqr_vec <- function(a, b) {
  res <- rep(NA_real_, length(a)); ok <- is.finite(a) & is.finite(b)
  res[ok] <- abs(a[ok] - b[ok])  # with two values, IQR == max-min
  res
}
...
# Persist masked sources (ensures on-disk tiling)
...
ysd_bap_out <- writeStart(ysd_bap_out, ysd_bap_path, overwrite=TRUE, wopt=wopt_flt)
ysd_nbr_out <- writeStart(ysd_nbr_out, ysd_nbr_path, overwrite=TRUE, wopt=wopt_flt)

# **OPEN** inputs for streaming reads
r_bap <- readStart(r_bap)
r_nbr <- readStart(r_nbr)

bs <- make_blocks(r_bap, chunk = 1024L)
message(sprintf("Predicting in %d blocks…", bs$n))

for (i in seq_len(bs$n)) {
  # ---- BAP: read, predict, write ----
  bap_blk <- terra::readValues(r_bap, row=bs$row[i], nrows=bs$nrows[i], mat=TRUE)
  pred_bap <- rep(NA_real_, nrow(bap_blk))
  ok_bap <- rowSums(is.finite(bap_blk)) == ncol(bap_blk)
  if (any(ok_bap)) {
    pred_bap[ok_bap] <- ranger::predict(
      model_bap, data = as.data.frame(bap_blk[ok_bap, , drop=FALSE]),
      num.threads = RF_THREADS_PRED
    )$predictions
  }
  terra::writeValues(ysd_bap_out, pred_bap, bs$row[i])
  
  # ---- NBR: read, predict, write ----
  nbr_blk <- terra::readValues(r_nbr, row=bs$row[i], nrows=bs$nrows[i], mat=TRUE)
  pred_nbr <- rep(NA_real_, nrow(nbr_blk))
  ok_nbr <- is.finite(nbr_blk[,1])
  if (any(ok_nbr)) {
    pred_nbr[ok_nbr] <- ranger::predict(
      model_nbr, data = data.frame(NBR = nbr_blk[ok_nbr,1]),
      num.threads = RF_THREADS_PRED
    )$predictions
  }
  terra::writeValues(ysd_nbr_out, pred_nbr, bs$row[i])
  
  if (i %% 10 == 0) message(sprintf("  block %d/%d", i, bs$n))
}

ysd_bap_out <- writeStop(ysd_bap_out)
ysd_nbr_out <- writeStop(ysd_nbr_out)
r_bap <- readStop(r_bap)
r_nbr <- readStop(r_nbr)
...
for (src_path in c(ysd_bap_path, ysd_nbr_path)) {
  ...
  out_int_path <- if (grepl("BAP", src_path)) ysd_bap_i_path else ysd_nbr_i_path
  out_yod_path <- if (grepl("BAP", src_path)) yod_bap_path   else yod_nbr_path
  
  r_int <- writeStart(r_int, out_int_path, overwrite=TRUE, wopt=wopt_i16)
  r_yod <- writeStart(r_yod, out_yod_path,   overwrite=TRUE, wopt=wopt_i16)
  r_src <- readStart(r_src)
  
  bs2 <- make_blocks(r_src, chunk = 2048L)
  for (i in seq_len(bs2$n)) {
    v <- terra::readValues(r_src, row=bs2$row[i], nrows=bs2$nrows[i], mat=TRUE)[,1]
    vint <- to_int_vec(v)
    vyod <- 1985 - vint
    terra::writeValues(r_int, vint, bs2$row[i])
    terra::writeValues(r_yod, vyod, bs2$row[i])
  }
  
  r_src <- readStop(r_src)
  r_int <- writeStop(r_int)
  r_yod <- writeStop(r_yod)
}
...
r_med <- writeStart(r_med, ens_median_path, overwrite=TRUE, wopt=wopt_flt)
r_agr <- writeStart(r_agr, agree_pairs_path, overwrite=TRUE, wopt=wopt_flt)
r_iqr <- writeStart(r_iqr, spread_iqr_path, overwrite=TRUE, wopt=wopt_flt)
r_sd  <- writeStart(r_sd,  spread_sd_path,  overwrite=TRUE, wopt=wopt_flt)
r_bap_pred <- readStart(r_bap_pred)
r_nbr_pred <- readStart(r_nbr_pred)

bs3 <- make_blocks(r_bap_pred, chunk = 2048L)
...
for (i in seq_len(bs3$n)) {
  ...
  terra::writeValues(r_med, med, bs3$row[i])
  terra::writeValues(r_agr, agr, bs3$row[i])
  terra::writeValues(r_iqr, iqr, bs3$row[i])
  terra::writeValues(r_sd,  sdv, bs3$row[i])
  
  if (i %% 10 == 0) message(sprintf("  CoE block %d/%d", i, bs3$n))
}

r_bap_pred <- readStop(r_bap_pred)
r_nbr_pred <- readStop(r_nbr_pred)
r_med <- writeStop(r_med)
r_agr <- writeStop(r_agr)
r_iqr <- writeStop(r_iqr)
r_sd  <- writeStop(r_sd)
