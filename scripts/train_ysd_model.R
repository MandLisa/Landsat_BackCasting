# ========================== BACKCAST YSD FROM 1985 (SAFE FULL SCRIPT) ===========================
# Global training (points), LOCAL inference on one BAP tile ONLY, restricted to forest (1/NA).
# Predictors: (1) BAP b1..b6, (2) NBR1985 (single band).
# Outputs: YSD (float + int), YOD (int), and CoE layers (median/agreement/spread).
# Crash-robust defaults (SAFE_MODE) avoid thread over-subscription and memory spikes.
# ================================================================================================
...
# --- CONFIG --------------------------------------------------------------------------------------
SAFE_MODE <- TRUE   # TRUE = conservative threading & on-disk processing; set FALSE after success to speed up
...
FOREST_MASK   <- "/mnt/eo/EFDA_v211/forest_landuse_aligned.tif"                                     # 1/NA
...
# SCALE HOOKS (usually keep as-is)
RESCALE_PTS_TO_0_1 <- FALSE   # if point bands are 0..10000 but rasters are 0..1
SCALE_RASTERS_BY   <- 1       # multiply raster values (e.g., 0.0001 or 10000 if needed)

# ENGINES
ENGINE_BAP <- "rf"  # "rf" or "xgb" for 6-band BAP model
ENGINE_NBR <- "rf"  # "rf" or "xgb" for single-band NBR model

set.seed(42)

# --- THREADING / MEMORY GUARDS -------------------------------------------------------------------
if (SAFE_MODE) {
  Sys.setenv(OMP_NUM_THREADS = "1",
             MKL_NUM_THREADS = "1",
             OPENBLAS_NUM_THREADS = "1",
             GDAL_NUM_THREADS = "1")
  options(mc.cores = 1, ranger.num.threads = 1)
  terraOptions(progress = 1, memfrac = 0.6)           # more on-disk ops
  # terraOptions(tempdir = "/mnt/eo/_tmp")            # set a fast temp directory if available
  CORES_TERRA <- 1
  NTHREAD_XGB <- 1
  NTHREAD_RF  <- 1
} else {
  # moderate parallelism; increase carefully
  Sys.setenv(OMP_NUM_THREADS = "4",
             MKL_NUM_THREADS = "4",
             OPENBLAS_NUM_THREADS = "4",
             GDAL_NUM_THREADS = "2")
  options(mc.cores = 4, ranger.num.threads = 4)
...
align_to_grid <- function(src, target, categorical = FALSE) {
  # project/resample src to target grid; nearest for categorical, bilinear otherwise
...
# Single-thread RF predict wrapper (stable under terra::predict tiling)
PRED_FUN_RF <- function(model, data, ...) {
  as.numeric(predict(model, data = as.data.frame(data), num.threads = NTHREAD_RF)$predictions)
}

# --- LOAD TRAINING DATA (GLOBAL) -----------------------------------------------------------------
...
if (ENGINE_BAP == "xgb") {
  ...
  params <- list(
    ...
    min_child_weight = best$min_child_weight, nthread = NTHREAD_XGB
  )
  model_bap <- xgb.train(params, d_all, nrounds = best$nrounds, verbose = 0)
  PRED_FUN_BAP <- function(model, data, ...) predict(model, as.matrix(data), nthread = NTHREAD_XGB)
  ...
  model_bap <- ranger(
    ...
    num.threads = NTHREAD_RF, seed = 42
  )
...
if (ENGINE_NBR == "rf") {
  ...
  PRED_FUN_NBR <- function(model, data, ...) predict(model, as.matrix(data), nthread = NTHREAD_XGB)
...
# align mask to BAP grid (nearest), crop to BAP extent, force 1/NA
...
# write masked sources (handy for inspection and more stable I/O)
...
# re-open from disk to force on-disk tiling in predictions (robust with large rasters)
...
ysd_ens_median <- app(
  ysd_stack, median, na.rm = TRUE, cores = CORES_TERRA,
  filename = file.path(OUT_DIR, "ysd_1985_tile_ensemble_median.tif"),
  overwrite = TRUE, wopt = wopt_flt
)

ysd_agree <- app(
  ysd_stack, agree_pairs, na.rm = TRUE, cores = CORES_TERRA,
  filename = file.path(OUT_DIR, "ysd_1985_tile_agreement_pairs.tif"),
  overwrite = TRUE
)

ysd_iqr <- app(
  ysd_stack, IQR, na.rm = TRUE, cores = CORES_TERRA,
  filename = file.path(OUT_DIR, "ysd_1985_tile_spread_IQR.tif"),
  overwrite = TRUE, wopt = wopt_flt
)

ysd_sd <- app(
  ysd_stack, sd, na.rm = TRUE, cores = CORES_TERRA,
