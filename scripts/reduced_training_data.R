...
# ======================================================================
# Add NNH zone information to the reduced training dataset
# Extract polygon attribute NNH at training point coordinates and add
# a human-readable region label:
#   1 -> Boreal
#   2 -> Temperate
#   3 -> Mediterranean
# ======================================================================
...
in_file   <- "/mnt/eo/EO4Backcasting/model_input/training_reduced_bap_nbrtrend.csv"
gpkg_file <- "/mnt/eo/EO4Backcasting/_data/biomes.gpkg"
out_file  <- "/mnt/eo/EO4Backcasting/model_input/training_reduced_bap_nbrtrend_biomes.csv"

# CRS deiner Trainingskoordinaten
training_crs <- "EPSG:3035"
...
if (!all(c("x", "y") %in% names(dt))) {
  stop("Input table must contain x and y.")
}

# Punkte als SpatVector
pts <- vect(dt, geom = c("x", "y"), crs = training_crs)
...
# Falls CRS unterschiedlich: Punkte transformieren
if (!same.crs(pts, nnh)) {
  pts <- project(pts, crs(nnh))
}

# ---------------------------- EXTRACT ----------------------------------

ex <- terra::extract(nnh["NNH"], pts)

# ex enthält meist eine ID-Spalte + NNH
dt[, NNH := ex$NNH]

dt[, nnh_region := fifelse(
  NNH == 1, "Boreal",
  fifelse(
    NNH == 2, "Temperate",
    fifelse(NNH == 3, "Mediterranean", NA_character_)
  )
)]

# ---------------------------- CHECKS -----------------------------------

cat("NA in NNH:", dt[is.na(NNH), .N], "\n")
cat("NA in nnh_region:", dt[is.na(nnh_region), .N], "\n")

print(dt[, .N, by = .(NNH, nnh_region)][order(NNH)])
